...
import type { Podcast, PodcastCategory } from '@/payload-types'
...
export const PodcastsListingClient: React.FC<Props> = ({
  heading,
  categories,
  activeCategorySlug,
  podcasts,
  ...
  return (
    <div className="container py-12 md:py-16">
      ...
      {podcasts.length === 0 ? (
        <p className="mt-12 text-black/60">No podcasts found.</p>
      ) : (
        <div className="mt-8 grid gap-x-8 gap-y-12 md:grid-cols-2 lg:grid-cols-3">
          {podcasts.map((p) => {
            const cat = typeof p.category === 'object' && p.category !== null ? p.category : null
            return (
              <Link key={p.id} href={`/podcasts/${p.slug}`} className="flex flex-col group">
                <div
